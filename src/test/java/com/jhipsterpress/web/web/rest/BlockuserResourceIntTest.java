...
public class BlockuserResourceIntTest {
     ...
     *
     * @see com.jhipsterpress.web.repository.search.BlockuserSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    @Test
    @Transactional
    public void searchBlockuser() throws Exception {
        // Initialize the database
        blockuserRepository.saveAndFlush(blockuser);
        ...
        restBlockuserMockMvc.perform(get("/api/_search/blockusers?query=id:" + blockuser.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(blockuser.getId().intValue())))
            .andExpect(jsonPath("$.[*].creationDate").value(hasItem(DEFAULT_CREATION_DATE.toString())));

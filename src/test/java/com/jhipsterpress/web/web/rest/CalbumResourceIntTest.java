...
public class CalbumResourceIntTest {
     ...
     *
     * @see com.jhipsterpress.web.repository.search.CalbumSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    @Test
    @Transactional
    public void searchCalbum() throws Exception {
        // Initialize the database
        calbumRepository.saveAndFlush(calbum);
        when(mockCalbumSearchRepository.search(queryStringQuery("id:" + calbum.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(calbum), PageRequest.of(0, 1), 1));
        // Search the calbum
        restCalbumMockMvc.perform(get("/api/_search/calbums?query=id:" + calbum.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(calbum.getId().intValue())))
            .andExpect(jsonPath("$.[*].creationDate").value(hasItem(DEFAULT_CREATION_DATE.toString())))

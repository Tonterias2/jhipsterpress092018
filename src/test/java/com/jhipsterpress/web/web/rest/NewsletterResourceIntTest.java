...
public class NewsletterResourceIntTest {
     ...
     *
     * @see com.jhipsterpress.web.repository.search.NewsletterSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    @Test
    @Transactional
    public void searchNewsletter() throws Exception {
        // Initialize the database
        newsletterRepository.saveAndFlush(newsletter);
        when(mockNewsletterSearchRepository.search(queryStringQuery("id:" + newsletter.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(newsletter), PageRequest.of(0, 1), 1));
        // Search the newsletter
        restNewsletterMockMvc.perform(get("/api/_search/newsletters?query=id:" + newsletter.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(newsletter.getId().intValue())))
            .andExpect(jsonPath("$.[*].creationDate").value(hasItem(DEFAULT_CREATION_DATE.toString())))

...
public class UprofileResourceIntTest {
     ...
     *
     * @see com.jhipsterpress.web.repository.search.UprofileSearchRepositoryMockConfiguration
     */
    @Autowired
    private UprofileSearchRepository mockUprofileSearchRepository;
    ...
    public void createUprofile() throws Exception {
        ...
        // Validate the Uprofile in Elasticsearch
        verify(mockUprofileSearchRepository, times(1)).save(testUprofile);
    }
    ...
    public void createUprofileWithExistingId() throws Exception {
        ...
        // Validate the Uprofile in Elasticsearch
        verify(mockUprofileSearchRepository, times(0)).save(uprofile);
    }
    ...
    public void deleteUprofile() throws Exception {
        ...
        // Validate the Uprofile in Elasticsearch
        verify(mockUprofileSearchRepository, times(1)).deleteById(uprofile.getId());
    }

    @Test
    @Transactional
    public void searchUprofile() throws Exception {
        // Initialize the database
        uprofileRepository.saveAndFlush(uprofile);
        when(mockUprofileSearchRepository.search(queryStringQuery("id:" + uprofile.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(uprofile), PageRequest.of(0, 1), 1));
        // Search the uprofile
        restUprofileMockMvc.perform(get("/api/_search/uprofiles?query=id:" + uprofile.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(uprofile.getId().intValue())))
            .andExpect(jsonPath("$.[*].creationDate").value(hasItem(DEFAULT_CREATION_DATE.toString())))
